...
const RADIUS_X = 360;
const RADIUS_Y = 240;
const COLLAPSE_MS = 900; // fly-into-Saturn duration
...
const Loader = ({ onComplete }) => {
  ...
  // Orbit (same speed always) + collapse-into-Saturn on exit
  useEffect(() => {
    ...
    const loop = (now) => {
      ...
      // Keep orbiting at the SAME speed even during collapse.
      angleRef.current += ((2 * Math.PI) / ORBIT_SECONDS) * dt;
      ...
      for (let i = 0; i < n; i++) {
        ...
        const baseX = Math.cos(theta) * RADIUS_X * t.rJitter;
        const baseY = Math.sin(theta) * RADIUS_Y * t.rJitter;

        const x = baseX * (1 - pull);
        const y = baseY * (1 - pull);
        const scale = 1 - pull * 0.92;
        el.style.transform =
          `translate(-50%, -50%) translate(${x}px, ${y}px) scale(${scale})`;
        el.style.opacity = String(1 - pull);
      }

      // Saturn collapses + fades in sync with the tiles (no lingering)
      if (saturnRef.current) {
        if (phaseRef.current === 'exiting') {
          const s = Math.max(1 - pull, 0);
          saturnRef.current.style.transform = `scale(${s})`;
          saturnRef.current.style.opacity = String(Math.max(1 - pull, 0));
        } else {
  ...
  const handleEnter = () => {
    ...
    // Music stays OFF by default — user enables it via the toggle only.
    setPhase('exiting');
    setTimeout(() => setVisible(false), COLLAPSE_MS + 80);
  };
  ...
  return (
    <>
      ...
      <AnimatePresence onExitComplete={() => onComplete && onComplete()}>
        {visible && (
          <motion.div
            key="loader"
            initial={{ opacity: 1 }}
          ...
          >
            ...
            {/* Saturn — solid white, collapses/fades with the tiles */}
            <div style={{
            ...
            {/* Click prompt — pinned to the very bottom, solid white */}
            <AnimatePresence>
              {phase !== 'exiting' && (
                <motion.div
            ...
            {/* Right — progress % (hides once loaded) */}
            <div style={{
